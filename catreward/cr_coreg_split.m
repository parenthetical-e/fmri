...
% Splits the rp_pavlov.txt in the WD into its parts (pavlov, coaster_localizaer, taskA and B), saving each.
%
% function cr_coreg_split(num),
...
% 103- rp_pavlov.txt 1254
    ...
    copyfile('rp_pavlov.txt', 'rp_all.txt');
        %% Move original to rp_all.txt

    rp = load('rp_pavlov.txt');
    ...
    save('rp_pavlov.txt', 'pav', '-ascii')
    save('rp_coaster_localizer.txt', 'cs', '-ascii')
    save('rp_taskA.txt', 'tA', '-ascii')
    save('rp_taskB.txt', 'tB', '-ascii')
